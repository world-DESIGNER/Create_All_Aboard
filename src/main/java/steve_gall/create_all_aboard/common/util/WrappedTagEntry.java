...
public record WrappedTagEntry(ResourceLocation id, boolean isTag, boolean isRequired)
{
	...
	public static WrappedTagEntry fromNbt(Tag tag)
	{
		CompoundTag compound = (CompoundTag) tag;
		ResourceLocation id = ResourceLocation.CODEC.decode(NbtOps.INSTANCE, compound.get("id")).result().get().getFirst();
		boolean isTag = compound.getBoolean("isTag");
		boolean isRequired = compound.getBoolean("isRequired");
		return new WrappedTagEntry(id, isTag, isRequired);
	}
